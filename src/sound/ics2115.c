...
static void update(void *param, INT16 **buffer, int length)
{
...
//double period = chip->timer[timer].preset* (chip->timer[timer].scale << 16) / 60000000000.0;
...
double counter;
	if(chip->timer[timer].scale) 
	{
		...
		counter = (((sc & 31)+1) * (chip->timer[timer].preset+1)) << (4+(sc >> 5));
		//period = 1000000000 * counter / 33868800;
		period = counter / 33868800;
	} 
	else period = 0;
	...
	if(chip->timer[timer].period != period) {
		...
		if(period)
			timer_adjust(chip->timer[timer].timer, TIME_IN_SEC(period), 0, TIME_IN_SEC(period));
		else
			timer_adjust(chip->timer[timer].timer, TIME_NEVER, 0, 0);
	}
}
...
{
//if (chip->osc == 2 && reg != 9 && reg != 0x4f)printf("ICS2115: write %02x, %02x:%d (%02x)\n", reg, data, msb, chip->osc);
chip->voice[chip->osc].tout = 40;
	...
	case 0x01: // [osc] Wavesample frequency
		...
		if(msb)
			...
			//if (chip->osc == 2)printf("%02x\n",chip->voice[chip->osc].fc);
		else
...
static void *ics2115_start(int sndindex, int clock, const void *config)
{
	...
	chip->timer[0].timer = timer_alloc(timer_cb_0);
	chip->timer[1].timer = timer_alloc(timer_cb_1);
    ...
    for (i = 0; i < 0x1000; i++) {
		   //chip->voltbl[i] = floor(14000.0 * pow(2.0,(((double)i/256 - 16) + 2)*1.1));
		   //chip->voltbl[i] = floor(pow(2.0,(((double)i/256 - 16) + 14.52)*1.1));
		   chip->voltbl[i] = floor(pow(2.0,(((double)i/256 - 16) + 14.7)*1.06));
		   //chip->voltbl[i] = floor(pow(2.0,(double)i/256));
		   //chip->voltbl[i] = floor(pow(2.0,(double)i/125-17));
		   //chip->voltbl[i] = floor((256 | (i & 255)) * 64) / pow(2.0 , (15 - (i>>8))*1.4);
    }
...
void ics2115_reset(void *_chip)
{
	...
	timer_adjust(chip->timer[0].timer, TIME_NEVER, 0, 0);
	timer_adjust(chip->timer[1].timer, TIME_NEVER, 0, 0);
