...
static data16_t *sprite_temp_render;
...
static void pgm_drawspriteline(int wide, data16_t* dest, int xzoom, int xgrow, int yoffset, int flip, int xpos)
{
	int xcnt,xcntdraw;
	int xzoombit;
	int xoffset;
...
static void pgm_drawsprite_new_zoomed(int wide, int high, int xpos, int ypos, int palt, int boffset, int flip, struct mame_bitmap* bitmap, UINT32 xzoom, int xgrow, UINT32 yzoom, int ygrow )
{
	int ycnt;
	int ydrawpos;
	data16_t *dest;
	...
	while (ycnt < high)
	{
		...
		if (yzoombit == 1 && ygrow == 1) // double this line
		{
			...
			{
				dest = (data16_t*)bitmap->line[ydrawpos];
...
static data16_t *pgm_sprite_source;

static void pgm_drawsprites(int priority, struct mame_bitmap* bitmap)
{
	/* ZZZZ Zxxx xxxx xxxx
       zzzz z-yy yyyy yyyy
       -ffp pppp Pvvv vvvv
    ...
    */
	...
	const data16_t *finish = pgm_spritebufferram+(0xa00/2);
	...
	while( pgm_sprite_source<finish )
	{
		...
		data16_t* pgm_sprite_zoomtable = &pgm_videoregs[0x1000/2];
